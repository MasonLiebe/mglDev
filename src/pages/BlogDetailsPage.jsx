...
const BlogDetailsPage = () => {
  ...
  return (
    <section className="container2 section text-white px-6">
      <Slide delay={0.1}>
      ...
      <article>
        <Slide
          className="flex lg:flex-row flex-col border-t dark:border-zinc-800"
          delay={0.1}
        >
          <div className="min-h-full w-1/2 max-lg:w-full lg:border-r border-r-0 dark:border-zinc-800 basis-3/4 pt-10 pb-4 lg:pr-6 px-0">
          ...
          <aside className="flex flex-col lg:max-h-full h-max gap-y-8 sticky top-20 bottom-auto right-0 basis-1/4 py-10 lg:px-6 px-0">
            <section className="border-b dark:border-zinc-800 border-zinc-200 pb-10">
              ...
              <address className="flex items-center gap-x-3 mt-4 not-italic">
                ...
                <div>
                  <h3 className="font-semibold text-zinc-200 text-lg tracking-tight">
                    {post?.author.name}
                  </h3>
                </div>
              </address>
              <div className="flex items-center gap-x-4 text-md mt-4 dark:text-zinc-400 text-zinc-600">
                <div className="flex items-center gap-x-2">
                  <BiCalendar />
                  <time
                    dateTime={moment(post?.publishedAt).format("mmm ddd yyyy")}
                  >
            ...
            {post?.tags !== undefined ? (
              <section className="border-b dark:border-zinc-800 pb-10">
                ...
                <ul className="flex flex-wrap items-center gap-2 tracking-tight">
                  {post?.tags.map((tag, i) => (
                    <li
                      key={i}
                      className="border dark:border-zinc-800 rounded-md px-2 py-1 text-sm"
                    >
                      {tag}
                    </li>
                  ))}
